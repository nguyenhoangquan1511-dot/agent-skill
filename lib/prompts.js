 ...
 *
 * Ý tưởng:
 *   Mặc định tick sẵn toàn bộ tool để user chỉ cần Enter là cài cho tất cả,
 *   ai muốn thu hẹp thì bỏ tick bớt.
 *
...
export async function promptToolSelection() {
  const response = await prompts({
    ...
    choices: TOOLS.map((tool) => ({ title: tool.label, value: tool.id, selected: true })),
    min: 1,
  });
 ...
 *
 * Ý tưởng:
 *   Mặc định chọn sẵn 'local' (thư mục project hiện tại) — lựa chọn an toàn hơn
 *   vì không đụng vào home directory.
 *
